...
class Venta
{
    ...
    {
        $all = $this->db
            ->query("SELECT IdVenta,IdTipoDocumento,Serie,Numero,(SELECT RazonSocial FROM cliente WHERE IdCliente=venta.IdCliente) as Empresa,
    ...
    {
        $all = $this->db
            ->query("SELECT IdVenta,IdTipoDocumento,Serie,(SELECT RazonSocial FROM cliente WHERE IdCliente=venta.IdCliente) as Empresa,
      Total,(SELECT ApellidoPaterno FROM usuario WHERE IdUsuario=venta.IdUsuario) AS Vendedor
    ...
    {
        $all = $this->db
            ->query("INSERT INTO venta (IdTipoDocumento,Serie,Numero,IdCliente,Total,IdUsuario)
    ...
    {
        $all = $this->db->query(
            "DELETE FROM  venta WHERE IdVenta = {$this->getIdVents()};"
        );
        return $all;
    }
    ...
    {
        $all = $this->db->query(
            "UPDATE venta SET IdTipoDocumento={$this->getIdTipoDocumento()}, Serie={$this->getSerie()},Numero={$this->getNumero()}, IdCliente={$this->getIdCliente()}  WHERE IdVenta={$this->getIdVents()};"
        );
        if ($all) {
            return true;
        }
