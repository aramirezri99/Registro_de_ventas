...
var myModal_delete = new bootstrap.Modal(document.getElementById('confirmModal'));
...
function mostrarProductos(){
    ...
    let res = await fetch("http://localhost/proyect-venta/producto/obtener_productos");
  ...
  function mostrarPrecio(){
    ...
    const getAll = async () =>{
      ...
      let res = await fetch("http://localhost/proyect-venta/producto/obtener_precio&id="+$valor+"");
  ...
  function LLenar_tabla() {
    ...
    $btnadd.addEventListener("click",function (e) {
      ...
      const $nombre =$select.querySelectorAll(`option`)[$idproducto-1].textContent;
...
function mostrarClientes(optioncliente){
      ...
      const getAll = async () =>{
          let res = await fetch("http://localhost/proyect-venta/cliente/mostrarclientes");
          let json = await res.json();
...
function RegistrarVenta(){
  ...
  $form.addEventListener("submit",function(e){
    ...
    const $table= d.querySelector("#tabla-detalles tbody");
      ...
      let obj = { Tipodocumento:options,serie:txtserie, numero:txtnumero, idcliente:selcliente, total:txttotal,usuario:txtuser};  
      ...
      let res = await fetch("http://localhost/proyect-venta/venta/registrar_venta", {
...
function RegistrarDetalle(){
  ...
  $detalle.forEach(function (el , index) {
    ...
    const insertar = async () =>{
        let res = await fetch("http://localhost/proyect-venta/detalle/registrar", {
        method:'POST',  headers: {"Content-type": "application/json"
        },
        body:JSON.stringify($obj)
       });
...
function show(id){
   ...
   const modal = async () =>{
            ...
            const res = await fetch("http://localhost/proyect-venta/venta/get_ventas_forid&id="+id); 
...
function  update() {
          ...
          const tipo =   d.querySelector('#seltipo-medit').value,
           ...
           id =d.querySelector('#idventa-medit').value;
            ...
            let obj = {id:id,tipo:tipo,serie:serie,numero:numero,cliente:cliente};

            const ajax = async () =>{


            const res = await fetch("http://localhost/proyect-venta/venta/edit_venta", { method:'POST',  headers: {"Content-type": "application/json" },
            body:JSON.stringify(obj)
            });
  ...
  document.addEventListener("click", (e) => {
    ...
    $btn.addEventListener("click",(e)=>{
                ...
                async function delet(){
                    ...
                    const res = await fetch("http://localhost/proyect-venta/venta/delete_venta&id="+id);
                    const respuesta = await res.json();

                    if (respuesta) {
                        setTimeout(function(){
                        myModal_delete.hide();
                        toastr.warning('El registro fue eliminado correctamente.', 'Eliminar Registro', {timeOut:0});
